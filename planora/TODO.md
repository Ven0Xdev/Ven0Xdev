...
- [ ] מסך העלאת קובץ (המודל, ה-DrawingProcessor ושכבת האחסון מוכנים; חסר ה-UI ונתיב הכתיבה)
...
- [x] מסך "כללי הפרויקט" (צפייה)
- [ ] עריכת כללי פרויקט מתוך הממשק
...
## פערים ידועים ב-Milestone 1
- העלאת קבצים מהממשק אינה ממומשת. גרסאות התוכנית נוצרות דרך שכבת השירות
  (`createChangeSetFromVersions`) ודרך ה-Seed. `StorageProvider` קיים אך ללא מימוש פעיל,
  ומחזיר שגיאה מפורשת במקום להיכשל בשקט.
- כללי פרויקט ניתנים לצפייה בממשק ומוגדרים ב-Seed; עריכה מתוך הממשק טרם נבנתה.
